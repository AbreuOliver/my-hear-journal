<script context="module">
  ...
  export const load = async ({ fetch }) => {
    return {
      props: {
        recentPosts: await fetch("/posts.json?limit=4").then((res) =>
          res.json()
        ),
      },
    };
  ...
  let currentWeekNumberOfTheYear = Math.ceil(
  ...
  let readingPlanResult = readingPlan.data.find(
    (e) => e.week === currentReadingWeek
  );
...
<div class="flex flex-col flex-grow">
  ...
  <div class="flex flex-col items-start justify-start text-xl h-full">
    ...
    <h4 style="color: #485163; margin-bottom: 1.75rem">
      Observations and study notes from the F260 Bible reading plan using the
      <a
        style="text-decoration: underline"
        target="_blank"
        href="https://replicate.org/what-is-a-hear-journal/"
        >H.E.A.R. journaling method</a
      >.<br /><br />
      <strong>Currently Reading Plan:</strong> <br />
      <h3 style="margin-bottom: 1.75rem">
        <strong
          ><span
            class="!text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-teal-500 dark:from-violet-500 dark:to-pink-500"
    ...
    <div
      style="margin-left: 1rem; margin-top: -2.15rem; margin-bottom: -2.5rem;"
    >
    ...
    <h4 style="color: #485163; margin-bottom: 1.75rem">
      <strong style="color: #485163;">Memory Verses:</strong> <br />
    </h4>
